...
class upload
{
	...
	public function tk(){
		...
		//判断是否存在该令牌信息
		if(! file_exists(PATH.$this->_tokenPath . $file['token'] . '.token')){
		
			$file['up_size'] = 0;                       //已上传文件大小
			$pathInfo = pathinfo($file['name']);
			$path = PATH.$this->_filePath . date('Ymd') .'/';
			//生成文件保存子目录
			if(! is_dir($path)){
				mkdir($path, 0700);
			}
		...
		if($fileInfo['size'] > $fileInfo['up_size']){
			...
			if(! empty($data)){
				...
				if($fileInfo['size'] > $fileInfo['up_size']){
					...
					//上传完成后删除令牌信息
					@unlink(PATH.$this->_tokenPath . $token . '.token');
				}
			}
		}
	...
	protected function setTokenInfo($token, $data){
		...
		file_put_contents(PATH.$this->_tokenPath . $token . '.token', json_encode($data));
	}
	...
	protected function getTokenInfo($token){
		$file = PATH.$this->_tokenPath . $token . '.token';
		if(file_exists($file)){
