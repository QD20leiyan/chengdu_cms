var TopBarConFig = {
	isShow : 1,
	isShowBan: {
        "zq.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            icp:'',
            community_id:0,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zq.yingxiong.com": {
            switch:1,
            kf_id:0,
            icp:'',
            community_id:0,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "szhl.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            icp:'',
            community_id:0,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "szhl.yingxiong.com": {
            switch:1,
            kf_id:0,
            icp:'',
            community_id:0,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zhs.demo.yingxiong.com": {
            switch:1,
            kf_id:180,
            community_id:33,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zhs.yingxiong.com": {
            switch:1,
            kf_id:180,
            community_id:33,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "super.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "super.yingxiong.com": {
            switch:1,
            kf_id:0,
            icp:'',
            community_id:0,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sg.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sg.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:39,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "hqwm.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "hqwm.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:46,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "ddd3.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:31,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "ddd3.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:31,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sszj.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:38,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sszj.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:38,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zg.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zg.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "bz.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "bz.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },

        "mw.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "mw.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "rw.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "rw.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "rw.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:19,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "tk.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "tk.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "tk.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fwy.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fwy.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fwy.zhengyuetech.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fwy.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fwy.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fwy.zhengyuetech.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "lzsg.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:26,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "qyzz.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:26,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "qyzz.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:26,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "wjyx.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "wjyx.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "wjyx.yingxiong.com": {
            switch:1,
            kf_id:27,
            community_id:27,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sd.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sd.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sd.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:28,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "csmsl.dev.yingxiong.com": {
            switch:1,
            kf_id:150,
            community_id:23,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "csmsl.demo.yingxiong.com": {
            switch:1,
            kf_id:150,
            community_id:23,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "csmsl.yingxiong.com": {
            switch:1,
            kf_id:150,
            community_id:23,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "dream.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "dream.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "dream.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zzyzf.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:37,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zzyzf.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:37,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "zzyzf.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:37,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "brdz.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "brdz.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "clqx.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "clqx.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fk.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fk.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "xsjs.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },


        "ca.yingxiong.com": {
            switch:1,
            kf_id:14,
            community_id:1,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "ca.demo.yingxiong.com": {
            switch:1,
            kf_id:14,
            community_id:1,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "hero.yingxiong.com": {
            switch:1,
            kf_id:157,
            community_id:24,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "hero.demo.yingxiong.com": {
            switch:1,
            kf_id:157,
            community_id:24,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "wpys.yingxiong.com": {
            switch:1,
            kf_id:14,
            community_id:16,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "wpys.demo.yingxiong.com": {
            switch:1,
            kf_id:14,
            community_id:16,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "fc.yingxiong.com":{
            switch:1,
            kf_id:100,
            icp:'',
            community_id:14,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b573b3c8901573c1ba4e908b3'
        },
        "fc.demo.yingxiong.com":{
            switch:1,
            kf_id:100,
            community_id:14,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b573b3c8901573c1ba4e908b3'
        },
        "fc.me.yingxiong.com":{
            switch:1,
            kf_id:100,
            community_id:14,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b573b3c8901573c1ba4e908b3'
        },
        "fc.demo.yingxiong.com":{
            switch:1,
            kf_id:100,
            community_id:12,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b573b3c8901573c1ba4e908b3'
        },
        "tqjd.demo.yingxiong.com":{
            switch:1,
            kf_id:100,
            community_id:12,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b573b3c8901573c1ba4e908b3'
        },
        "tqjd.dev.yingxiong.com":{
            switch:1,
            kf_id:100,
            community_id:12,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b573b3c8901573c1ba4e908b3'
        },
        "tqjd.yingxiong.com":{
            switch:1,
            kf_id:100,
            community_id:12,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b573b3c8901573c1ba4e908b3'
        },
        "wctt.yingxiong.com": {
        	switch:1,
        	kf_id:2,
        	community_id:5,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c54ec9f2a01550577c9b855ee'
        },
        "dfzj.yingxiong.com":{
        	switch:1,
        	kf_id:70,
        	community_id:7,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c5419944701544cfe96c07595'
        },
        "dfzj.demo.yingxiong.com":{
            switch:1,
            kf_id:70,
            community_id:7,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08c5419944701544cfe96c07595'
        },
        "moba.yingxiong.com":{
        	switch:1,
        	kf_id:82,
        	community_id:22,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c5678ff3f01567d6e5b9110ea'
        },
        "y.yingxiong.com":{
        	switch:1,
        	kf_id:71,
        	community_id:8,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c5419944701542db9244d157f'
        },
        "yx.demo.yingxiong.com":{
            switch:1,
            kf_id:'&nbsp;',
            community_id:'&nbsp;',
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08c5419944701542db9244d157f'
        },
        "yingxiong.com":{
            switch:1,
            kf_id:' ',
            community_id:' ',
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08c5419944701542db9244d157f'
        },
        "tank.yingxiong.com":{
        	switch:1,
        	kf_id:66,
        	community_id:10,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c53cc2ed60153cfbc3e1203cd'
        },
        "ddd2.yingxiong.com": {
        	switch:1,
        	kf_id:73,
        	community_id:9,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c53cc2ed60153d10b447e08d8'
        },
        "ddd2.demo.yingxiong.com": {
            switch:1,
            kf_id:73,
            community_id:9,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08c53cc2ed60153d10b447e08d8'
        },
        "gf.yingxiong.com":{
        	switch:1,
        	kf_id:34,
        	community_id:'',
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c4fb67fd1014feef2012d7157'
        },
        "xm.yingxiong.com":{
        	switch:1,
        	kf_id:30,
        	community_id:4,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08b52302266015235b8ea2c0ccd'
        },
        "quan.yingxiong.com":{
        	switch:1,
        	kf_id:26,
        	community_id:'',
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08c518ba15a0151902852d20d94'
        },
        "qtdl.yingxiong.com": {
        	switch:1,
        	kf_id:25,
        	community_id:11,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08b5084e7c40150a7cdbf304d02'
        },
        "xsg.yingxiong.com":{
        	switch:1,
        	kf_id:5,
        	community_id:'',
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08b4ef125d5014f263fee32710c'
        },
        "mx.yingxiong.com":{
        	switch:1,
        	kf_id:32,
        	community_id:20,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08b50137eac0150647c6e1f7801'
        },

        "sm.me.yingxiong.com":{
            switch:1,
            kf_id:126,
            community_id:17,
            icp:'<p class="f_txt"><a href="http://sq.ccm.gov.cn/ccnt/sczr/service/business/emark/gameNetTag/2c94c08c5f6b1133015f6bd45057094c">文网游备字〔2017〕M-RPG 1549 号 \n 新广出审[2017]8939号</a></p>',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b50137eac0150647c6e1f7801'
        },
        "sm.demo.yingxiong.com":{
            switch:1,
            kf_id:126,
            community_id:17,
            icp:'<p class="f_txt"><a href="http://sq.ccm.gov.cn/ccnt/sczr/service/business/emark/gameNetTag/2c94c08c5f6b1133015f6bd45057094c">文网游备字〔2017〕M-RPG 1549 号 \n 新广出审[2017]8939号</a></p>',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b50137eac0150647c6e1f7801'
        },
        "sm.yingxiong.com":{
            switch:1,
            kf_id:126,
            community_id:17,
            icp:'<p class="f_txt"><a href="http://sq.ccm.gov.cn/ccnt/sczr/service/business/emark/gameNetTag/2c94c08c5f6b1133015f6bd45057094c">文网游备字〔2017〕M-RPG 1549 号 \n 新广出审[2017]8939号</a></p>',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b50137eac0150647c6e1f7801'
        },
		"tstd.me.yingxiong.com":{
            switch:1,
            kf_id:54,
            community_id:'',
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b50137eac0150647c6e1f7801'
        },
        "tstd.demo.yingxiong.com":{
            switch:1,
            kf_id:54,
            community_id:'',
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b50137eac0150647c6e1f7801'
        },
        "tstd.yingxiong.com":{
            switch:1,
            kf_id:54,
            community_id:'',
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b50137eac0150647c6e1f7801',
			company_show:1
        },
		"fk.yingxiong.com": {
        	switch:1,
        	kf_id:119,
        	community_id:16,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
		"fk.demo.yingxiong.com": {
        	switch:1,
        	kf_id:119,
        	community_id:16,
        	icp:'',
        	wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
        	gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "cc.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:15,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "cc.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:15,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "cc.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:15,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "rts.yingxiong.com": {
            switch:1,
            kf_id:14,
            community_id:16,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "jws.dev.yingxiong.com": {
            switch:1,
            kf_id:38,
            community_id:13,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "jws.demo.yingxiong.com": {
            switch:1,
            kf_id:38,
            community_id:13,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "jws.yingxiong.com": {
            switch:1,
            kf_id:38,
            community_id:13,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "sourcecc.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "jj.demo.yingxiong.com": {
            switch:1,
            kf_id:132,
            community_id:18,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "jj.yingxiong.com": {
            switch:1,
            kf_id:132,
            community_id:18,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },

        "cd.dev.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:22,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "cd.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:22,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "cd.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:22,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },

        "cc.yingxiong.com": {
            switch:1,
            kf_id:124,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "lzl.yingxiong.com": {
            switch:1,
            kf_id:124,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },

        "xsqs.yingxiong.com": {
            switch:1,
            kf_id:124,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },

        "xsqs.demo.yingxiong.com": {
            switch:1,
            kf_id:124,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },

        "sswd.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },

        "sswd.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "lszt.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "lszt.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:0,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "yqlmx.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:20,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "yqlmx.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:20,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "nba.demo.yingxiong.com": {
            switch:1,
            kf_id:92,
            community_id:21,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "nba.yingxiong.com": {
            switch:1,
            kf_id:92,
            community_id:21,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "kok.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:47,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "kok.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:47,
            icp:'',
            wwwIcon_id:'4f6d0e4a76ec4ec1baa5b91e75404393',
            gameIcon_id:'4028c08b54eca17b0154ecb9f77203c5'
        },
        "acewar.demo.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:42,
            icp:'',
            aurl: "javascript:;",
            burl: "javascript:;"
        },
        "acewar.yingxiong.com": {
            switch:1,
            kf_id:0,
            community_id:42,
            icp:'',
            aurl: "javascript:;",
            burl: "javascript:;"
        },
    },
    website: {
        "clqx.*.yingxiong.com": 59,
        "fc.*.yingxiong.com": 24,
        "brdz.*.yingxiong.com": 65,
        "jj.*.yingxiong.com": 25,
        "sm.*.yingxiong.com": 26,
        "ddd2.*.yingxiong.com": 27,
        "cc.*.yingxiong.com": 28,
        "sourcecc.*.yingxiong.com": 28,
        "tstd.*.yingxiong.com": 29,
        "sjsgz.*.yingxiong.com": 29,
        "fk.*.yingxiong.com": 30,
        "yqlmx.*.yingxiong.com": 31,
        "xsqs.*.yingxiong.com": 33,
        "cd.*.yingxiong.com": 34,
        "ca.*.yingxiong.com": 35,
        "jws.*.yingxiong.com": 37,
        "wctt.*.yingxiong.com": 38,
        "y.*.yingxiong.com": 39,
        "dfzj.*.yingxiong.com": 40,
        "qtdl.*.yingxiong.com": 41,
        "xm.*.yingxiong.com": 42,
        "zjlm.*.yingxiong.com": 43,
        "tank.*.yingxiong.com": 43,
        "mx.*.yingxiong.com": 44,
        "gf.*.yingxiong.com": 45,
        "acewar.*.yingxiong.com": 48,
        "wpys.*.yingxiong.com": 49,
        "lzl.*.yingxiong.com": 57,
        "hero.*.yingxiong.com": 62,
        "xsjs.*.yingxiong.com": 63,
        "zzyzf.*.yingxiong.com": 66,
        "csmsl.*.yingxiong.com": 67,
        "dream.*.yingxiong.com": 68,
        "sd.*.yingxiong.com": 73,
        "wjyx.*.yingxiong.com": 75,
        "qyzz.*.yingxiong.com": 74,
        "fwy.zhengyuetech.com": 78,
        "fwy.*.yingxiong.com": 78,
        "tk.*.yingxiong.com": 80,
        "nba.*.yingxiong.com": 85,
        "rw.*.yingxiong.com": 52,
        "sszj.*.yingxiong.com": 84,
        "ddd3.*.yingxiong.com": 86,
        "hqwm.*.yingxiong.com": 88,
        "sg.*.yingxiong.com": 87,
        "super.*.yingxiong.com": 92,
        "zhs.*.yingxiong.com": 94,
        "szhl.*.yingxiong.com": 93,
        "zq.*.yingxiong.com": 95,
        "heroim.com": 46,
        "lszt.*.yingxiong.com": 98,
        "nba.*.yingxiong.com": 85,
        "kok.*.yingxiong.com": 100,
    },
	link : 'http://cdnstatic.yingxiong.com/footer/css/style.css'
};
var fgw_wap_down='';
var fgw_wap_tap='';
var fgw_wap_hy='';
var fgw_wap_down1='';
var fgw_wap_tap1='';
var fgw_wap_hy1='';
var wap_xz = $(".stat_wap_down").length; 
var wap_tap_xz = $(".stat_wap_tap").length; 
var wap_hy_xz = $(".stat_wap_hy").length; 
var wap_cover_xz = $(".stat_cover_wap_down").length; 
var wap_cover_tap_xz = $(".stat_cover_wap_tap").length; 
var wap_cover_hy_xz = $(".stat_cover_wap_hy").length; 
var wap_channel="";
var fgw_url = window.location.href;
var url_id=fgw_url.indexOf("/part");
if (TopBarConFig.isShow &&TopBarConFig.isShowBan[window.location.host].switch) {
	var icp=TopBarConFig.isShowBan[window.location.host].icp;
    	! function () {
        var t = function (t) {
            return document.getElementById(t)
        }
        var host = window.location.host;
        if(host.indexOf('yingxiong.com')) {
            var arr = host.split('.');
            var all_host = arr[0] + '.*.yingxiong.com';
            var website = TopBarConFig.website[all_host];
        } else {
            var website = TopBarConFig.website[window.location.host];
        }
            if(website==85 || website==39 || website==44 || website==43 || website==38 ||website==45 || website==42|| website==41){
                var url='http://cc.yingxiong.com/commonMethod/ajax-website.html';
            }else{
                var url='/commonMethod/ajax-website.html';
            }
            $.get(url, {
                id: website
            }, function(data) {
                if(data.status == 0) {
                    wap_channel="fgw_" + data.channel;
                    fgw_wap_down="fgw_" + data.channel + "_wap_down";
                    fgw_wap_tap="fgw_" + data.channel + "_tap";
                    fgw_wap_hy="fgw_" + data.channel + "_hy";
                    fgw_wap_down1="." + "fgw_" + data.channel + "_wap_down";
                    fgw_wap_tap1="." + "fgw_" + data.channel + "_tap";
                    fgw_wap_hy1="." + "fgw_" + data.channel + "_hy";
                    var fgw_name_url="fgw_" + data.channel;
                    if(data.flag==1 && url_id >= 0){
                        $('.channel_name').css('display','block');
                        $('.channel_tap_name').css('display','none');
                        $('.no_channel').css('display','none');
                        $(".stat_wap_down").addClass(fgw_wap_down);
                        $(".stat_wap_tap").addClass(fgw_wap_tap);
                        $(".stat_wap_hy").addClass(fgw_wap_hy);
                        $(".stat_cover_wap_down").addClass(fgw_wap_down);
                        $(".stat_cover_wap_tap").addClass(fgw_wap_tap);
                        $(".stat_cover_wap_hy").addClass(fgw_wap_hy);
                        $(fgw_wap_down1).removeClass('stat_wap_down');
                        $(fgw_wap_tap1).removeClass('fgw_wap_tap');
                        $(fgw_wap_hy1).removeClass('fgw_wap_hy');
                         $(fgw_wap_down1).removeClass('stat_cover_wap_down');
                        $(fgw_wap_tap1).removeClass('stat_cover_wap_tap');
                        $(fgw_wap_hy1).removeClass('stat_cover_wap_hy');
                        $("body").on("click",fgw_wap_down1,function(){HLog.event(fgw_wap_down)});
                        $("body").on("click",fgw_wap_tap1,function(){HLog.event(fgw_wap_tap)});
                        $("body").on("click",fgw_wap_hy1,function(){HLog.event(fgw_wap_hy)});
                        var timer = setInterval(function () {
                            if (!window.HLog) {
                                num++;
                                if (num >= 10) {
                                    clearInterval(timer);
                                }
                                console.log(num);
                            } else {
                               HLog.push(fgw_name_url);
                                clearInterval(timer);
                            }
                        }, 500);
                    }else if(data.flag==2 && url_id >= 0){
                        $('.channel_name').css('display','block');
                        $('.channel_tap_name').css('display','none');
                        $('.no_channel').css('display','none');
                        $(".stat_wap_down").addClass(fgw_wap_down);
                        $(".stat_wap_tap").addClass(fgw_wap_tap);
                        $(".stat_wap_hy").addClass(fgw_wap_hy);
                        $(".stat_cover_wap_down").addClass(fgw_wap_down);
                        $(".stat_cover_wap_tap").addClass(fgw_wap_tap);
                        $(".stat_cover_wap_hy").addClass(fgw_wap_hy);
                        $(fgw_wap_down1).removeClass('stat_wap_down');
                        $(fgw_wap_tap1).removeClass('fgw_wap_tap');
                        $(fgw_wap_hy1).removeClass('fgw_wap_hy');
                        $(fgw_wap_down1).removeClass('stat_cover_wap_down');
                        $(fgw_wap_tap1).removeClass('stat_cover_wap_tap');
                        $(fgw_wap_hy1).removeClass('stat_cover_wap_hy');
                        $("body").on("click",fgw_wap_down1,function(){HLog.event(fgw_wap_down)});
                        $("body").on("click",fgw_wap_tap1,function(){HLog.event(fgw_wap_tap)});
                        $("body").on("click",fgw_wap_hy1,function(){HLog.event(fgw_wap_hy)});
                        var timer = setInterval(function () {
                            if (!window.HLog) {
                                num++;
                                if (num >= 10) {
                                    clearInterval(timer);
                                }
                                console.log(num);
                            } else {
                               HLog.push(fgw_name_url);
                                clearInterval(timer);
                            }
                        }, 500);
                    }
                    // $('head').append(data.msg.web_count);
                    $('meta[name="csrf-token"]').attr('content',data.csrf);
                    if(wap_xz > 0) {
                         $('.stat_wap_down').click(function(){HLog.event('stat_wap_down')});
                        }
                        if(wap_tap_xz>0){
                            $('.stat_wap_tap').click(function(){HLog.event('stat_wap_tap')});
                        }
                        if(wap_hy_xz>0){
                            $('.stat_wap_hy').click(function(){HLog.event('stat_wap_hy')});
                        }
                        if(wap_cover_xz > 0) {
                         $('.stat_cover_wap_down').click(function(){HLog.event('stat_cover_wap_down')});
                        }
                        if(wap_cover_tap_xz>0){
                            $('.stat_cover_wap_tap').click(function(){HLog.event('stat_cover_wap_tap')});
                        }
                        if(wap_cover_hy_xz>0){
                            $('.stat_cover_wap_hy').click(function(){HLog.event('stat_cover_wap_hy')});
                        }
                }
            }, 'json');
        !new function () {
            var node = document.createElement("link");
            node.setAttribute("rel","stylesheet");
            node.setAttribute("href",TopBarConFig.link);
            document.body.appendChild(node);
        }
        !new function () {
            var node = document.createElement("footer");
            node.id = "Hero-bar";
            if (TopBarConFig.isShowBan[window.location.host].kf_id == 0) {
                var kefu = "javascript:alert('敬请期待!')";
            } else {
                var kefu = "//kf.yingxiong.com/Mobile/checkOption?Gid="+TopBarConFig.isShowBan[window.location.host].kf_id;
            }

            if (TopBarConFig.isShowBan[window.location.host].community_id == 0) {
                var sequ = "javascript:alert('敬请期待!')";
            } else {

                if (window.location.host == 'jj.yingxiong.com'
                    || window.location.host == 'jj.demo.yingxiong.com'
                ) {
                    var sequ = 'https://bbs.yingxiong.com/t/m/18';
                } else {
                    // var sequ = "//bbs.yingxiong.com/index/gameBbsLogin?id="+TopBarConFig.isShowBan[window.location.host].community_id;
                    var sequ = "//bbs.yingxiong.com/T/M/"+TopBarConFig.isShowBan[window.location.host].community_id;
                }
            }
			
			//show company
			var company = '';
			try{
				if(TopBarConFig.isShowBan[window.location.host].company_show == undefined 
				|| TopBarConFig.isShowBan[window.location.host].company_show){
					company = '<a href="http://www.yingxiong.com/protect.html" style="margin:10px 0;display:block;font-size: 12px;color: #969696;text-align: center;text-decoration: underline;">点击查看家长监护工程</a>';
				}
			}catch(e){}
			
            node.innerHTML = '<div class="f_t"><ul><li><a href="'+kefu+'" id="kefu">客服中心</a></li><li><a href="'+sequ+'" id="community">游戏社区</a></li></ul></div><p class="f_txt">版权所有：北京卓越晨星科技有限公司</p><p class="f_txt">京ICP备15026730号-2</p><p class="f_txt">COPY RIGHT   @2015-2018  ALL RIGHTS RESERVED</p><p class="f_txt" style="margin-bottom:10px;">联系方式：010-50948585</p>' +
                '<p class="f_link"><a href="//sq.ccm.gov.cn/ccnt/sczr/service/business/emark/toDetail/4f6d0e4a76ec4ec1baa5b91e75404393">《网络文化经营许可证》 京网文[2015]0629-259号</a></p>' +
                icp;
//              company;
            document.body.appendChild(node);
            // <div class="tel">
            // <a href="tel:4009393333">
            // <i></i>
            // 400-939-3333
            // </a>
            // </div>
            
        }
        // $.get('/commonMethod/ajax-count.html',{},function(data){
            //     if(data.status==0){
            //         $('head').append(data.data);
            //     }
            // },'json');
    }();
    if(TopBarConFig.isShowBan[window.location.host].community_id==''){
        $("footer .f_t ul li:nth-child(2)").css("visibility","hidden");
        
    }
    if(TopBarConFig.isShowBan[window.location.host].kf_id==''){
        $("footer .f_t ul li:nth-child(1)").css("visibility","hidden");
    }
}




