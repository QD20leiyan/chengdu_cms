	...
	function getStyle(obj, attr){
		...
		for(i=0;i<aLi.length;i++){
			...
			aLi[i].onclick = function(){
				with(oFrame.style){display = 'block',top = this.offsetTop +'px',left = this.offsetLeft +'px',width = '236px',height = '418px';}
				// oFrame.innerHTML = '<img src="images/'+(this.index + 1)+'.jpg" />';
				oFrame.innerHTML = '<img src="'+aImg[this.index].src+'" />';
				var oImg = oFrame.getElementsByTagName('img')[0];
				var iWidth = parseInt(oImg.width);
				var iHeight = parseInt(oImg.height);
				var iLeft = parseInt((document.documentElement.clientWidth / 2) - 118);
				var iTop = parseInt((document.documentElement.clientHeight / 2) - 209);
				with(oImg.style){height = width = '100%';}
				startMove(oFrame, {opacity:100, left:iLeft, top:iTop, width:'236px', height:'418px'});
		...
		aBli[0].onclick = function(){
			...
			oFrame.innerHTML = '<img src="'+aImg[iNow-1].src+'" width="236px" height="418px" />';
		};
		...
		aBli[2].onclick = function(){
			...
			oFrame.innerHTML = '<img src="'+aImg[iNow-1].src+'" width="100%" height="100%" />';
		};
