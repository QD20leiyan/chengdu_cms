		...
		if(data.status == 2){
		  ...
		  _key = data.p_id;
			...
			return false;
			 ...
			 var img_url = 'http://dev.static.yingxiong.com/dfzj/2.0/common/images/1212/'+data.p_id+'.png';
			$('#p_img').attr('src',img_url);
			if(data.p_id == 1){
				var p_name = '银币*388888';
				var p_level = '安慰奖';
			}else if(data.p_id == 2){
				var p_name = '技能书礼包';
				var p_level = '三等奖';
  ...
  MtimeSet = setTimeout(function() {
    ...
    if(k>=j){
      ...
      zui(_speed + 10,j,k);
		...
		var img_url = 'http://dev.static.yingxiong.com/dfzj/2.0/common/images/1212/'+k+'.png';
		  $('#p_img').attr('src',img_url);
		  if(k == 1){
			  var p_name = '银币*388888';
			  var p_level = '安慰奖';
