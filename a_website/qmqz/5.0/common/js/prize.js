      ...
      function(data){
		...
		if(data.status == 2){
		  ...
		  _key = data.p_id;
			...
			var img_url = 'http://dev.static.yingxiong.com/qmqz/4.0/common/images/1212/'+data.p_id+'.png';
			...
			if(data.p_id == 1){
				...
				var p_name = '死亡骑士碎片（2个）';
				var p_level = '二等奖';
			}else if(data.p_id == 6){
				var p_name = '荒漠SCAR（7天），傻熊背包（7天），300荣誉';
				var p_level = '三等奖';
			}else if(data.p_id == 7){
				var p_name = '电竞钢锋';
				var p_level = '特等奖';
			}else if(data.p_id == 8){
				var p_name = '幽魂AA-12（1天），200荣誉';
				var p_level = '四等奖';
			}
  ...
  MtimeSet = setTimeout(function() {
    ...
    if(k>=j){
      ...
      zui(_speed + 10,j,k);
		...
		var img_url = 'http://dev.static.yingxiong.com/qmqz/4.0/common/images/1212/'+k+'.png';
		  $('#p_img').attr('src',img_url);
		  if(k == 1){
			  var p_name = '炼狱M14（3天）';
			  var p_level = '二等奖';
