(function() {
	...
	tcss.prototype = {
		init: function(e) {
			...
			_l = _d.location, _b = _d.body, _tt = []
		},
		run: function() {
			var e, t, n;
			e = (new Date).getTime(), _cookie.init(), this.url.push(this.getDomainInfo()), this.url.unshift("http://pingfore." + _domainToSet + "/pingd?"), this.url.push(this.getRefInfo(_params));
			try {
			...
			this.url.push(this.getMainEnvInfo()), this.url.push(this.getExtendEnvInfo()), this.url.push("&vs=" + _ver), _params.userDefineVariable ? this.url.push(_udv.setv(_params.userDefineVariable)) : this.url.push(_udv.setv()), _cookie.setCookie("ssid"), _cookie.save(), _params.originalReferer && this.url.push("&or=" + _params.originalReferer), t = (new Date).getTime(), _params.extParam ? n = _params.extParam + "|" : n = "", this.url.push("&ext=" + escape(n + (t - e))), this.url.push("&reserved1=" + escape(_params.reserved1Param || "")), this.url.push("&rand=" + Math.round(Math.random() * 1e5));
			var i = this.getSud();
			i && _tt.push("su=" + escape(i.substring(0, 256))), this.url.push("&tt=" + escape(_tt.join(";"))), this.sendInfo(this.url.join("")), this.url[0] = "http://iedtcss.qq.com/heatmap?", this.watchHot(this.url.join("")), _params.hot && (document.attachEvent ? document.attachEvent("onclick", function(e) {
				pgvWatchClick(e)
		...
		getDomainInfo: function(e) {
			var t, n;
			_curDomain = t = _params.virtualDomain || _l.host, t = _curDomain.toLowerCase(), _domainToSet || (_domainToSet = this.getCookieSetDomain());
			if(e) {
				var r = t.indexOf(":");
				r > -1 ? t = t.substr(0, r) + ".hot" + t.substr(r) : t += ".hot"
			}
			return n = this.getCurrentUrl(), "dm=" + t + "&url=" + n
		},
		...
		getRefInfo: function(e) {
			...
			_refDomain = e.virtualRefDomain ? e.virtualRefDomain : "", _refUrl = e.virtualRefURL ? e.virtualRefURL : "";
			if(e.statIframe || e.useCookie == "true") {
				i = _cookie.get("pgvReferrer=");
				var l = _d.URL,
					c = l.indexOf("?");
				c > -1 && (l = l.substr(0, c)), _cookie.set("pgvReferrer", l)
			} else if(e.useCookie == "set" && _refDomain != "" && _refUrl != "") {
				var l = "https:" == _l.protocol ? "https://" : "http://";
				l += _refDomain + refUrl, _cookie.set("pgvReferrer", l)
			} else if(e.useCookie != "set" || a == _n && f == _n)
				if(e.useCookie == "get") {
					var h = _cookie.get("pgvReferrer=");
					h != "" && (i = h), _cookie.set("pgvReferrer", "")
				} else _cookie.set("pgvReferrer", "");
			else {
				var l = "https:" == _l.protocol ? "https://" : "http://";
				l += a == _n ? _curDomain : a, l += f == _n ? _curUrl : f, _cookie.set("pgvReferrer", l)
			}
			s = e.tagParamName || "ADTAG", o = this.getParameter(s, _d.URL), o && (t = "ADTAG", n = o), u = i.indexOf("://");
			if(u > -1 && t == _n) {
				var p = /(\w+):\/\/([^\:|\/]+)(\:\d*)?(.*\/)([^#|\?|\n]+)?(#.*)?(\?.*)?/i,
					d = i.match(p);
				d && (t = d[2], n = d[4] + d[5])
			...
			_refDomain != "" && e.useCookie == "false" && (t = _refDomain), _refUrl != "" && e.useCookie == "false" && (n = _refUrl), _refDomain = t, _refUrl = escape(n);
			var v = _n,
				m = _n;
			v = _cookie.get("ied_rf=", !0), m = _cookie.get("ied_qq=", !0);
			if(v == _n || t == "ADTAG" || "--" == v || v.indexOf("ADTAG") == -1 && t != _n) {
				var g = t + n;
				g.length > 128 && (g = g.substring(0, 128)), v = g;
				var y = new Date;
				y.setDate(y.getDate() + 1), y.setHours(y.getHours() - y.getHours()), y.setMinutes(y.getMinutes() - y.getMinutes()), y.setSeconds(y.getSeconds() - y.getSeconds());
		...
		getMainEnvInfo: function() {
			...
			try {
				var t = _n,
					...
					u = 0,
					a = navigator;
				self.screen && (t = screen.width + "x" + screen.height, n = screen.colorDepth + "-bit"), a.language ? r = a.language.toLowerCase() : a.browserLanguage && (r = a.browserLanguage.toLowerCase()), u = a.javaEnabled() ? 1 : 0, i = a.cpuClass, s = a.platform, o = (new Date).getTimezoneOffset() / 60, e = "&scr=" + t + "&scl=" + n + "&lang=" + r + "&java=" + u + "&cc=" + i + "&pf=" + s + "&tz=" + o
			} catch(f) {} finally {
				return e
			}
		},
		...
		getCookieSetDomain: function(e) {
			...
			t = i.length, n = _curDomain.indexOf(".cn"), n > -1 && t--, r = "qq.com", t == 1 ? r = _curDomain : t > 1 && (r = _curDomain.substring(i[t - 2] + 1));
			if(e) switch(t) {
				case 1:
				case 2:
					r = _curDomain;
		...
		watchClick: function(e) {
			try {
				...
				if(t) {
					...
					i.url.push(i.getDomainInfo(!0)), i.url.push("&hottag=" + escape(n)), i.url.push("&hotx=" + s.x), i.url.push("&hoty=" + s.y), i.url.push("&rand=" + Math.round(Math.random() * 1e5)), i.url.unshift("http://pinghot." + _domainToSet + "/pingd?"), i.sendInfo(i.url.join(""))
				}
			} catch(u) {}
		},
		...
		getPos: function(e, t, n, r) {
			...
			return s.x = s.x - o + u - (Math.max(document.body.scrollHeight, document.documentElement.scrollHeight) > (typeof window.innerHeight == "undefined" ? document.documentElement.clientHeight : window.innerHeight) ? 8.5 : 0), s
		},
		sendHeat: function(e, t) {
			e += "&x=" + t.x + "&y=" + t.y + "&r3=" + Math.max(document.body.scrollHeight, document.documentElement.scrollHeight), (new Image).src = e
		},
		...
		heapClick: function(e, t) {
			var n = function(e, t, n) {
					var r = function(e) {
						e = window.event || e, target = e.srcElement || e.target, n(e, target)
					};
		...
		sendInfo: function(e) {
			_image = new Image(1, 1), _image.src = e
		}
	};
	var _udv = {
			vscope: {
			...
			setv: function(e) {
				var t = "",
					n = "";
				if(!e || !e.name || e.name == "" || !e.value || e.value == "" || !e.scope || e.scope < 1 || e.scope > 3) n = _cookie.get("custvar=") == _n ? _cookie.get("custvar=", !0) : _cookie.get("custvar="), t = "&custvar=" + n;
				else {
		...
		_cookie = {
			...
			setCookie: function(e, t, n) {
				...
				return t ? this.saveCookie(e + "=" + r, "expires=Sun, 18 Jan 2038 00:00:00 GMT;") : this.set(e, r), r
			},
			save: function() {
				...
				var i = "";
				e && (i = "expires=" + e.toGMTString()), this.saveCookie(t, i)
			},
