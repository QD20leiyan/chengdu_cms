$(function (){
        ...
        $.post("/site/ajax-login.html",{
            ...
            if(data.status == 0){
                ...
                if(person.is_yuyue == 0){
                    yue();
                    // $('.bigDialog').hide();
                    // $('#dialog2').find('p').text('京东10~200元优惠券');
                    // $('#dialog2').find('a').attr('href',data.msg);
                    // $('#dialog2').show();
                    // $('.mask').show();
                    // $('.dialogBox').hide();
                    // $('#success').find('.loginTitle').show();
                    // $('#success').find('.r_main').css({top:'150px'});
                    // $('#getPriseBox').show();
                }else{
                    $('.mask').hide();
                    $('.dialogBox').hide();
    ...
    //拉取预约人数
    function getyueNum() {
        var num = parseInt($('.text_number').eq(0).text());
        jindu(parseInt(num));
    }
    getyueNum();
    //拉取用户信息
    ...
    function jindu(temp){
        var percent;
        if(temp<100000){
            percent = (temp / 10000).toFixed(2) + '%';
        }
        if(temp >= 100000){
            percent = (((temp - 100000) / 300000).toFixed(2) * 25 + 8) + '%';
            $('.item_word').find('li:nth-child(1)').addClass('active');
        }if(temp >= 400000){
            percent = (((temp - 400000) / 300000).toFixed(2) * 26 + 33) + '%';
            $('.item_word').find('li:nth-child(2)').addClass('active');
        }if(temp >= 700000){
            percent = (((temp - 700000) / 300000).toFixed(2) * 26 + 59) + '%';
    ...
    function allUserPrise() {
        ...
        $.get("/site/ajax-lottery-log.html", {}, function(data) {
            ...
            if(data.data.length <= 0){
                $(priseWord).addClass('noDate');
                prise += "<li style='text-align: center'>暂无数据</li>";
            }
    ...
    function yue() {
        ...
        $.post("/site/ajax-yuyue.html", params, function(data) {
            if(data.status == 0){
                ...
                $('.bigDialog').hide();
                $('#dialog2').find('p').text('京东200-10元优惠券');
                $('#dialog2').find('a').attr('href',data.msg);
                $('#dialog2').show();
                $('.mask').show();
                $('.dialogBox').hide();
    ...
    $('.open_draw').click(function () {
        ...
        if(person.invite_count >= 400){
            $('#success').find('.loginTitle').hide();
            $('#success').find('.r_main').css({top:'140px'});
            $('#success').find('.r_main').find('p').text('');
        ...
        if(person.today_draw_count >= 10){
            $('#success').find('.loginTitle').hide();
            $('#success').find('.r_main').css({top:'100px'});
            $('#success').find('.r_main').find('p').text('');
            ...
            $.post("/site/ajax-lottery.html", params, function(data) {
                if(data.status == 0){
                    ...
                    if(data.id == 3){
                        $('.myGift').attr('class','myGift gift3');
                        $('#dialog1').show();
                    }
                    if(data.id == 4){
                    ...
                    if(data.id == 7){
                        $('#dialog2').find('p').text(data.gift_name);
                        $('#dialog2').find('a').attr('href',data.gift_code);
                        $('#dialog2').show();
                    }
                    if(data.id == 8){
                        $('#dialog2').find('p').text(data.gift_name);
    ...
    $('.open_draw').mouseenter(function () {
        $(this).children('.pool_master').addClass('animate');
    })
    $('.open_draw').mouseleave(function () {
        $(this).children('.pool_master').removeClass('animate');
    })
